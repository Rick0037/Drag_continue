...
% Problem
    ...
    parameters.EvaluationFunction = 'external';
        ...
        ProblemParameters.InputNumber = 3; % number of sensors       
            % si(t)
            ProblemParameters.NumberSensors = 3;
             %   SensorsEval{ProblemParameters.InputNumber} = '0';
             SensorsEval=cell(1,ProblemParameters.NumberSensors);
             for p = 1:ProblemParameters.NumberSensors
              ...
              SensorsEval{1} = ['Cd(T)'];
              SensorsEval{2} = ['Cl(T)'];
              SensorsEval{3} = ['Cldelay(T)'];   
                ProblemParameters.Sensors = SensorsEval; % name in the problem
            % hi(t)
            ProblemParameters.NumberTimeDependentFunctions = 0;
             % ProblemParameters.TimeDependentFunctions{2,:} = {'h(T)'};% syntax in the problem (if null then comment)
        ...
        % Evaluation
        ProblemParameters.Tmax = [500 750];
        ...
        % Costs        
        ProblemParameters.J0 = 1.56; % no actuation  %maybe it will change to the wake stability 
        ProblemParameters.Jmin = 0.8627; % best boat-tailing  %and this is the drag 
        ProblemParameters.Jmax = inf;
        % Round evaluation
        ProblemParameters.EstimatePerformance = 'last'; % default 'mean', if drift 'last'
        ProblemParameters.gamma = [1,0,0,0,0,0]; % Jb,Jc,Jd,Je,Jf,Jg
        % Path for external evaluation
        ProblemParameters.PathExt = '../PinballMF/OUTPUT/Costs';
